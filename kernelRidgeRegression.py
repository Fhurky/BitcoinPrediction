...
try:
    data = pd.read_csv('Data/btcusd.csv', engine='python', on_bad_lines='skip')  # Updated line
...
# Reduce dataset size by sampling (e.g., 100,000 samples)
data_sampled = data.sample(n=5000, random_state=42)
Y = data_sampled['Close']
X = data_sampled.drop(['Close'], axis=1)

# Split the data into training and testing sets
train_x, test_x, train_y, test_y = train_test_split(X, Y, test_size=0.3, random_state=0)

KernelRidgeModel = KernelRidge(kernel='linear', degree=2, alpha=0.5)
KernelRidgeModel.fit(train_x, train_y)


# Save the Model
try:
    with open('Models/KernelRidge_regression_model.pkl', 'wb') as file:
        pc.dump(KernelRidgeModel, file)
    print("Model saved successfully.")
except Exception as e:
    print(f"Error saving the model: {e}")

# Make predictions using the test set
predictions = KernelRidgeModel.predict(test_x)
...
config.set('Metrics', 'KernelRidgeRegression_MSE-score',str(mean_squared_error(test_y, predictions)))
...
def prediction(unknown):
    ...
    return KernelRidgeModel.predict(unknown)
